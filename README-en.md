...
- Whenever the Knight either takes normal enemy/contact damage, lands a real nail parry, or pulls off a valid shadow-dash dodge on the `3rd` dash physics step, the system advances `1` round and every active orb triggers its own `passive` once
...
- Black gets the most value when you can keep rounds advancing consistently, trigger passives, build a large stored orb, and then force an evocation at the right moment
...
- If you want faster passive triggers, plan around damage, parries, and shadow-dash dodges that advance rounds instead of only pre-storing orbs
